 ...
 * 상세 페이지 처리 인스턴스 정의하기
*/
const CartViewPagingReload = () => {
    // 파라메터(Parameter)를 담을 상태변수 정의
    //  - useParams() : 외부로부터 전송받은 파라메터를 담고 있음
    //  - 사용자가 선택한 페이지번호를 담고 있는 page 파라메터 추가
    const {cart_no, cart_prod, page} = useParams();
    ...
    const handleDelete = () => {
        ...
        deleteCart(cart_no, cart_prod)
            ...
            .then(() => {
                ...
                // navigate('/cart/list');
                navigate('/cart/list_paging_reload?page=' + page);
            })
            .catch((err) => {
        ...
        <div>
            ...
            {mode === 'view' && (
                <div>
                    ...
                    <button onClick={() => navigate('/cart/list_paging_reload?page=' + page)}>목록</button>
                </div>
            )}
        </div>
    );
...
export default CartViewPagingReload;