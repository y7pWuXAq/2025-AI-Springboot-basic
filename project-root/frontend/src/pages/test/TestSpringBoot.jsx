...
const TestSpringBoot = () => {
    ...
    useEffect(() => {
        ...
        getTest()
            // then() : SpringBoot 서버에 요청 처리 진행
            //  - 비동기 방식으로 처리 : 페이지 이동 없이 통신으로만 주고 받음
            .then((res) => {
                // 응답 받은 데이터를 testResultMessage 상태변수에 저장하기
                setTestResultData(res.data);
            })
            // 예외 처리
            .catch((err) => {
                // 콘솔에 출력
                console.error(">>>>>>> SrpingBoot 연결 실패 : ", err);
            });
    ...
    return (
        <div>
            ...
            {/* Home 바로가기 버튼 생성하기 */}
            <div>
                {/* 클릭을 하면 root(/) 경로로 페이지 변경 처리 : navigate("/") */}
                <button onClick={() => navigate("/")}>HOME 바로가기</button>
            </div>

            <hr/>
            {/* TestSpringBootFrom 컴포넌트 렌더링(포함)하기 */}
